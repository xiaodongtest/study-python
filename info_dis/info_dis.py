...
keyword = ['xxxxxxxxxxxx中心', 'xxxXX喜奔', '小xxx','xxxxxx办卡','xxxxxxxxx','xxxxxxxxxxxx' ,'上海xxx东xxx展xxxxxx'] # 关键字列表
keywords = ['阿里巴巴','阿里云']# 测试列表
SHODAN_API_KEY = "ugzKocaJMch22p8ywabAlGQWLCYxXXXX"
urlcrazy=['xxx.com.cn','xxx.net','xxx.com.cn','xxx.com.cn','sbqbd.com.cn','shbfbk.com.cn','shfzbk.com.cn','xxxd.com.cn']
api = shodan.Shodan(SHODAN_API_KEY)#本想用shodan api的，后来xxx现实在太少，干脆就直接搜索就得了
def dy_shodan():
    codel = []
...
def dy_fofa():# fofa的钓鱼搜索
    ...
    for line in keyword:
        ...
        url = 'https://fofa.so/api/v1/search/all?email=26025!@#$%^@qq.com&key=!#@$#$%^^^399d7fd0d7cfce9320&qbase64='+ s#此处换成自己的api接口
        #print url
        try:
            res =requests.get(url,headers=headers,timeout=5)
...
def Check():#zoomeye的登录
    ...
    data_info = {'username': '260258$%^^^@qq.com', 'password': '@#$%^^&***(**'}#此处换成自己的账户密码
...
def urlcrazy_fin():#钓鱼域名探测
    ...
    for line in urlcrazy:
        target = line.strip()
        print target
        a = subprocess.Popen('urlcrazy -o urlcrazy.txt ' + target, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,shell=True, cwd='/root/xxx/diaoyu')
...
def aizhan_chaxun():
    ...
    payload = {'ddlSearchMode': '2', 'host': '上海xxx东xxx展xxx股份有限公司xxx中心'}
